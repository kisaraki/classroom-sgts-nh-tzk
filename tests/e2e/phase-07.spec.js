...
test("three data-driven levels switch without leaking dashboard state", async ({
  ...
  await expect(page.locator("[data-level-remaining]")).toHaveText("216 小時 00 分鐘");
  await expect(page.locator(".objective-list > li")).toHaveCount(6);
  await expect(page.locator("#storm-position")).toContainText("北緯 13.60°");
  ...
  await expect(page.locator("[data-level-remaining]")).toHaveText("360 小時 00 分鐘");
  await expect(page.locator(".objective-list > li")).toHaveCount(5);
  await expect(page.locator("#storm-position")).toContainText("北緯 16.00°");
