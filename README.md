...
- 動態冷水尾流；六站完整模型觀測以地圖內高密度有色毛玻璃卡呈現，預設
  分置低干擾空白區，並可用滑鼠、觸控筆、單指或鍵盤移動及個別歸位。
- 地圖支援滑鼠滾輪、拖曳、雙指縮放與平移、可見按鈕及鍵盤操作；
  視角不會修改物理、步數或 fingerprint。
- localStorage v1 安全回復，以及後台使用的嚴格 JSON 驗證與重播能力。
- 後台保留 CSV、模擬 JSON、沙盒設定 JSON、Canvas PNG 與文字摘要輸出，
  不在玩家介面顯示。
- 低／中／高粒子層級與減少動態模式；效能量測保留為內部測試能力，不在
  玩家畫面顯示開發診斷。
...
海岸線繪製後快取於離屏 Canvas。六站觀測卡疊加在地圖上，以正規化畫面
座標保存玩家拖曳位置；定位點與引線仍依真實經緯度及相機重投影，卡位不
進入物理、重播或 fingerprint。玩家地圖點選查詢介面已取消；純座標轉換及
測地函式仍供鏡頭與模型使用。
