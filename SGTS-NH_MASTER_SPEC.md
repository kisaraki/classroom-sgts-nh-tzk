...
地圖視覺以具來源與授權紀錄的 Natural Earth II 真實地形陰影柵格貼圖呈現
島嶼及陸地；貼圖以 1:10m 陸地多邊形建立透明海岸遮罩，必須同源靜態部署，
不得在執行期依賴外部圖磚服務。貼圖只負責視覺，海陸判定、地形分區及物理
計算仍以經驗證的教育用地理資料為準；影像載入失敗時必須降級使用簡化地形，
不得使模擬停止或進入錯誤狀態。
...
- 第一、二象限：主 Canvas 擬真地圖位於上方並橫跨整個內容寬度，關卡目標、
  模擬時間、六站模型觀測毛玻璃卡、地圖視角控制與地圖說明隨地圖呈現。
- 六站觀測卡必須以有色、半透明模糊背景的 HTML 區塊呈現，並以真實測站
  定位點及引線對應；預設卡位應分置於中國大陸內陸、南側新幾內亞附近等
  低干擾空白區，不得包圍臺灣、琉球或颱風主航跡。卡片採高密度兩欄資料
  排版，但仍須完整顯示測站名稱、持續風、最大陣風、當前雨率、累積雨量、
  與颱風中心距離、地形修正及模擬更新時間，不得以不明縮寫或極小字取代。
- 玩家可用滑鼠、觸控筆或單指個別拖曳六站觀測卡；卡位以正規化畫面座標
  保存於呈現層，須夾限地圖界內、避免互相重疊並避開地圖控制。地圖縮放或
  平移時卡位維持，定位點與引線依真實經緯度重投影；定位點另置於卡片上方
  圖層，經過卡片時仍須可見且不得反向推動卡位。拖曳事件
  不得觸發地圖平移，也不得修改測站 `lon`／`lat`、觀測、物理、步數、重播、
  匯出或 fingerprint。Home 鍵或按兩下可將個別卡片恢復預設位置。
- 六張卡不因放大或平移而消失；視角外測站的定位點夾限於地圖邊緣。
  窄螢幕採兩欄緊湊配置，淺橫向畫面採四欄兩列並保留右下控制區；核心
  欄位標籤與數值字級分別不得低於 10 px 與 11 px。
- 玩家前端不提供點選地圖查詢、查詢面板或選取游標。
- 第三象限：下方左側以戰況資訊為主，包含颱風、環境因子與教育聲明。
- 玩家畫面不得顯示 FPS、固定步長、步進次數、模型更新耗時、長工作數量、
  顯示像素比例、環境網格數、部署平台、fingerprint 或其他開發診斷；效能
  與重播驗證資料保留於不進入視覺及無障礙樹的內部量測節點與測試工具。
...
2. 海溫或環境底圖。
3. 經緯線。
4. Natural Earth II 真實地形貼圖、陸地與教育判定海岸線。
5. 壓力場及副高。
6. 導引風箭頭。
7. 冷水尾流。
8. 降雨。
...
12. 粒子。
13. 目標區與警戒區。
14. 必要的載入與錯誤提示；不得常駐顯示開發診斷。

Canvas 上方另以 DOM 疊加六站模型觀測卡、定位引線與地圖視角控制；
觀測卡位置可由玩家拖曳，但只屬呈現狀態，這些元件不得寫回任何物理狀態。
...
- 不依賴 hover 才能取得重要資訊。
- 滑桿需支援鍵盤與觸控。
- 地圖視角縮放範圍為 1～4 倍，中心必須限制於東經 100°～160°、
  北緯 0°～40°的可視世界邊界內。
- 滑鼠滾輪或觸控板以指標下的地理位置為錨點縮放；滑鼠、觸控筆與單指
  拖曳可平移，雙指手勢可同時縮放與平移。
- 指標起始於六站觀測卡時只移動該卡，不得同時平移地圖；拖曳期間不得每一
  畫格重新量測 DOM 幾何，放開後的位置須能隨地圖尺寸正規化重排。
- 必須提供可見的放大、縮小與重設視角按鈕；Canvas 聚焦時以加號、減號、
  方向鍵及 `Home` 或 `0` 提供鍵盤等價操作。
- 視角中心、縮放與有效地理邊界只屬呈現狀態，不得改變颱風位置、風速、
  測站觀測、關卡步數、重播、資料匯出或 fingerprint。

## 14.7 無障礙

- 所有控制項有 `label`。
- 鍵盤可操作。
- 六站觀測卡可聚焦；方向鍵每次微調位置，Shift 加方向鍵可加大步距，
  Home 鍵恢復預設位置，Escape 鍵取消進行中的拖曳。
- 焦點狀態清楚。
- 不只靠顏色表示狀態。
- Canvas 旁提供文字摘要。
...
- 粒子亂數與物理亂數可使用不同種子流。
- 關閉粒子後，物理結果完全一致。
- 颱風主體與粒子外圍尺寸須依 `active` 與 `structureStage` 呈現可辨識的
  生成、成熟、衰減及低於熱帶性低氣壓強度差異；視覺縮放不得寫回或取代
  物理暴風半徑。
- 第一版位於北半球；Canvas 的 Y 軸向下，因此氣旋視覺角度必須隨時間遞減，
  使粒子在畫面呈逆時針旋轉。移動方向角不是渦旋自轉，不得一併反號。
- 減少動態模式停用動態粒子，且不得改變物理狀態、重播指紋或關卡結果。
...
- 隱藏分頁時暫停渲染。
- 切回時不無限制補算。
- 內部保留 FPS、update／render duration 與 Long Task 診斷供測試量測，
  但不得在正式玩家畫面或 Canvas 上顯示。
...
- 桌面三區、平板上下排列、窄螢幕提示。
- 44 px 以上觸控目標。
- 開發測試可透過內部量測節點取得 FPS、模擬時間、狀態與地圖範圍；
  正式玩家畫面不得常駐顯示這些開發診斷。
...
> 歷史追蹤註記（2026-08-06）：上述「點選或觸控地圖查詢」及其驗收是
> 2026-07-30 Phase 2 的真實已通過內容，不回頭改寫。Phase 9 後依
> `DEC-0039` 取消玩家前端查詢介面與選取游標；經緯度雙向轉換、
> 測地純函式及 `GEO-POINTER-001` 的歷史 passed 證據仍然有效。
...
- 第一版結案後重新開啟並完成本機 `v1.0.1` 介面維護候選；候選狀態為
  `completed`，已依明確操作授權部署至 Pages，尚未取得產品 `approved`、未建立
  版本標籤或 GitHub Release；Phase 9 `approved`、`v1.0.0` 發布史實與 Phase 10
  維持 `pending` 且未執行的狀態均不改變。
- 六站觀測卡預設移至中國大陸內陸及南側新幾內亞附近低干擾區，卡片位置
  與真實測站定位點解耦，新增指標拖曳、鍵盤微調、個別歸位、邊界夾限、
  卡片碰撞與地圖控制避讓；呈現狀態不進入物理、步數、重播或 fingerprint。
- 卡片重排為高密度完整氣象欄位；窄螢幕採兩欄，淺橫向採四欄兩列並保留
  右下控制空間；定位點置於卡片上方的獨立圖層，避免相機移動推動卡位。
- 正式玩家畫面移除效能診斷、固定步長／步次、fingerprint、Canvas 診斷框
  與模型中間值；內部效能量測、無障礙 Canvas 摘要及測試能力仍保留。
...
- 六站模型觀測移入地圖，以有色毛玻璃資訊卡、測站定位點及引線呈現。
- 取消玩家前端地圖查詢、查詢面板與選取游標，保留純地理座標轉換能力。
- 地圖新增滑鼠滾輪、拖曳、雙指縮放／平移、可見按鈕及鍵盤等價操作；
  視角狀態與正式物理、重播及 fingerprint 完全隔離。

### 1.0.4｜2026-08-06

- 颱風圖像尺寸依生命史結構與是否維持熱帶性低氣壓強度呈現可辨識變化，
  但不改動物理暴風半徑或模型版本。
- 明確固定北半球氣旋在 Canvas 畫面呈逆時針旋轉。
- 島嶼與陸地改用 Natural Earth II 1:10m 真實地形陰影貼圖；同源靜態部署，
  海陸與地形物理判定仍以教育用地理資料為準，並保留安全降級。
