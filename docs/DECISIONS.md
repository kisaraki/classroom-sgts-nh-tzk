...
## DEC-0038｜生命史視覺、北半球旋轉與真實地形貼圖

- 日期：2026-08-06。
- 狀態：accepted。
- 颱風尺寸：保留 `galeRadius` 的正式物理用途與模型版本；渲染端另以
  `active`、`structureStage` 及物理半徑計算有界視覺半徑。生成、成熟、
  衰減與低於熱帶性低氣壓強度必須可辨識，但不得寫回物理狀態或重播資料。
- 旋轉：第一版位於北半球；Canvas Y 軸向下，所以粒子角度隨模擬時間遞減，
  畫面呈逆時針。颱風移動方向角只決定圖形朝向，不代表渦旋自轉。
- 地形：以 Natural Earth II 1:10m 陰影地形與 Natural Earth 1:10m 陸地
  多邊形產生 2,400×1,600 透明 WebP，保留真實島嶼地貌與海岸細節，同源
  部署且不使用執行期外部圖磚。GeoJSON 與解析式臺灣地形仍是物理權威。
- 降級：地形影像為可選視覺資產；404、解碼失敗或尺寸不符時沿用原簡化
  地形，模擬不得進入 `ERROR`。影像載入後必須使固定地圖快取失效重建，
  離屏 Canvas 依裝置像素比建立，並保持 PNG 匯出不受跨來源污染。

## DEC-0039｜六站地圖觀測卡、玩家查詢退場與地圖鏡頭

- 日期：2026-08-06。
- 狀態：accepted。
- 六站觀測：那霸、臺北、臺中、日月潭、花蓮與澎湖繼續由正式
  `ObservationModel` 每 10 模擬分鐘產生持續風、最大陣風、當前雨率及
  累積雨量；`StationMapOverlay` 只負責將不可變快照呈現成地圖內的
  有色毛玻璃 HTML 卡。六張卡不因縮放而消失；測站離開當前可見地理範圍
  時，定位點夾限於地圖邊緣，仍以引線對應卡片。
- 玩家查詢：依使用者最新指示與主規格第 0.6 節，取消前端點選地圖
  查詢、查詢面板、查詢鍵盤行為及選取游標。經緯度雙向轉換、Haversine
  距離及最近測站等純函式繼續保留。
- 歷史證據：Phase 2 完成 commit `2a426c4`、批准 commit `26cde99` 與
  `GEO-POINTER-001 passed` 仍為真實歷史；本決策只取代現行玩家介面，
  不將歷史改寫為失敗或未實作。
- 地圖鏡頭：`MapCamera` 保存 1～4 倍縮放、中心與有效邊界；
  `MapInteractionController` 統一處理滾輪、拖曳、雙指、可見按鈕與鍵盤操作。
